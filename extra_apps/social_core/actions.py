...
def do_complete(backend, login, user=None, redirect_name='next',
    ...
    response.set_cookie('name', jwt_payload_handler(pyload), max_age=24*60*60)
    response.set_cookie('token', user.name if user.name else user.username, max_age=24*60*60)
    return response
